...
\echo '==== ANÁLISIS PARA MIGRACIÓN DE JSONB A CAMPOS DIRECTOS ===='
...
SELECT 
    ...
    array_to_string(array_agg(DISTINCT c."botConfig"->>'language'), ', ') as valores_ejemplo
FROM 
    "Client" c
WHERE 
...
SELECT 
    k as campo,
    'Requiere tabla separada o normalización' as recomendacion,
    jsonb_typeof(c."botConfig" -> k) as tipo_actual,
    COUNT(*) as num_clientes_con_campo
FROM 
    "Client" c,
...
\echo 'ALTER TABLE "Client" ADD COLUMN IF NOT EXISTS "workingDays" VARCHAR(100);'
\echo 'ALTER TABLE "Client" ADD COLUMN IF NOT EXISTS "botPhone" VARCHAR(20);'
\echo 'ALTER TABLE "Client" ADD COLUMN IF NOT EXISTS "botEmail" VARCHAR(100);'
\echo 'ALTER TABLE "Client" ADD COLUMN IF NOT EXISTS "botWebsite" VARCHAR(255);'
...
\echo 'UPDATE "Client" SET "botPhone" = "botConfig"->>'phone' WHERE "botConfig" ? 'phone';'
\echo 'UPDATE "Client" SET "botEmail" = "botConfig"->>'email' WHERE "botConfig" ? 'email';'
\echo 'UPDATE "Client" SET "botWebsite" = "botConfig"->>'website' WHERE "botConfig" ? 'website';'
\echo 'UPDATE "Client" SET "workingDays" = "botConfig"->>'workingDays' WHERE "botConfig" ? 'workingDays';'
